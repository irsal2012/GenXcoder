...
class AgentExecutionResponse(BaseModel):
    ...
    message: str = Field(..., description="Status message")
    result: Optional[Any] = Field(None, description="Execution result if completed")
    error: Optional[str] = Field(None, description="Error message if failed")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "result": {"code": "def fibonacci(n): return n if n <= 1 else fibonacci(n-1) + fibonacci(n-2)"},
                "started_at": "2025-01-07T12:53:00Z",
                "completed_at": "2025-01-07T12:53:05Z"
            }
        }
...
class PipelineExecutionResponse(BaseModel):
    ...
    status: StatusEnum = Field(..., description="Execution status")
    message: str = Field(..., description="Status message")
    result: Optional[Any] = Field(None, description="Pipeline execution result if completed")
    progress: Optional[ProgressResponse] = Field(None, description="Current progress information")
    error: Optional[str] = Field(None, description="Error message if failed")
    started_at: Optional[datetime] = Field(None, description="Execution start time")
    completed_at: Optional[datetime] = Field(None, description="Execution completion time")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "success": True,
                "execution_id": "pipe-123e4567-e89b-12d3-a456-426614174000",
